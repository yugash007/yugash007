<div align="center">

<img src="https://raw.githubusercontent.com/ABSphreak/ABSphreak/master/gifs/Hi.gif" width="70"/>

<h1>Hi, I'm Devineni Yugash 👋</h1>

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=24&pause=1000&color=2E9EFF&center=true&vCenter=true&width=750&lines=B.Tech+AI+%26+ML+Student;Java+%7C+Python+%7C+Problem+Solver;Building+Responsible+AI;Discipline+%2B+Consistency+%2B+Depth" />

</div>

---

<div align="center">
<i>“Real engineering is about clarity, consistency, and depth.”</i>
</div>

---

## 👨‍💻 About Me

I am an undergraduate student specializing in **Artificial Intelligence & Machine Learning** at **Mohan Babu University**.  
My focus is on **first-principles thinking**, **strong computer science foundations**, and **building scalable, ethical technology** that serves both people and nature.

- 🔭 **Currently working on:** Advanced DSA in Java & Python ML foundations  
- 🌱 **Learning:** System design fundamentals & backend architecture  
- 🎯 **Goal:** Industry-ready by 2026 (Google / NVIDIA / Microsoft)  
- 🧘 **Core values:** Satya (Truth), Ahimsa (Compassion), Discipline  

---

## 🎯 Current Focus (2025–2026)

<img align="right" width="360" src="https://user-images.githubusercontent.com/74038190/229223263-cf2e4b07-2615-4f87-9c38-e37600f8381a.gif"/>

- **📊 Data Structures & Algorithms**
  - Java (patterns, optimization, interview readiness)
- **🤖 Artificial Intelligence & Machine Learning**
  - Python foundations, ML logic, data handling
- **💻 Core Computer Science**
  - C programming (memory, pointers, systems thinking)
- **🌐 Web Fundamentals**
  - HTML & CSS (semantic, structure-first development)
- **🔧 Engineering Workflow**
  - Git & GitHub (clean commits, readable history)

---

## 🛠️ Tech Stack

<div align="center">

### 👨‍💻 Languages
<img src="https://skillicons.dev/icons?i=java,python,c&theme=dark" />

### 🤖 AI & Data
<img src="https://skillicons.dev/icons?i=numpy,pandas,tensorflow&theme=dark" />

### 🌐 Web
<img src="https://skillicons.dev/icons?i=html,css&theme=dark" />

### ⚙️ Tools
<img src="https://skillicons.dev/icons?i=git,github,linux,vscode&theme=dark" />

</div>

---

## 🚀 Featured Projects

| Project | Description |
|------|------------|
| **DSA-with-Java-Roadmap** | Structured roadmap with patterns and problem-solving strategy |
| **AI Arena (Hackathon Project)** | Modular AI platform concept with scalable architecture |
| **ML Foundations** | Data analysis & predictive modeling notebooks |
| **Frontend Mini Projects** | Clean, responsive UI experiments using HTML & CSS |

---

## 🌱 Learning Philosophy

Learn deeply.
Build slowly.
Think clearly.
Act consistently.

I don’t chase trends.
I invest in fundamentals that compound.

---

## 🤝 Connect With Me

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/yugash0007)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/yugash007)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:yugash05@gmail.com)

📄 **Resume:** In progress

</div>

---

<div align="center">

```text
╔══════════════════════════════════════════════════════╗
║  I am not here to look impressive.                  ║
║  I am here to build real skill and honest depth.    ║
║                                                      ║
║  If you value discipline and long-term thinking,    ║
║  we will resonate.                                  ║
╚══════════════════════════════════════════════════════╝
'''
</div>

**सत्यमेव जयते**
<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=110&section=footer" width="100%"/> 
